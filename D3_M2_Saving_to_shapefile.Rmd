...
head(acad_exo_wide) # Created at the end of Data Wrangling Part II.

acad_19 <- joinLocEvent(park = "ACAD", from = 2019, to = 2019)

acad_for_shp <- merge(acad_19[ , c("Plot_Name", "X_Coord", "Y_Coord")], acad_exo_wide, by = "Plot_Name", 
                      all.x = TRUE, all.y = TRUE)

# Note that if we had used the code below to generate acad_exo_wide, the coordinates would already be in there
acad_exo_wide <- acad_exospp %>% select(Plot_Name, X_Coord, Y_Coord, Latin_Name, spp_pres) %>% 
