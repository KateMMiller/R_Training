...
plot(bounds)
```
To prepare the shapefile polygons for plotting in ggplot, we have to fortify them to make them into a spatial data frame. 
```{r d2, echo=T}
...
```{r, d3, echo=T}
ggmap <- ggplot()+
         coord_sf(crs = 26919)+  # set coordinate system to NAD83 UTM Zone 19N      
         ...
         geom_point(data = wet_rich_df, 
                           aes(x = long, 
                               ...
                               color = Site_Type),
                            shape = 17)+
         theme_void() # because I don't like ggplot's grids in default grid
...
```{r, d4, echo=T, eval=FALSE}
install.packages('leaflet')
```
```{r, d4b, echo=F, results='hide'}
