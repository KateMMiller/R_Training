...
You can also check if any Site_Name records are missing a dash.
```{r, c21k, echo=T, results='show'}
wetdat$Site_Name[which(!grepl("-", wetdat$Site_Name))]
```

We'll fix both of the issues we found below:
...
In the case of the wetdat, there should be 508 total rows, 7 sites, and three different years. I will show you different ways to check that in the code below. I also often skim the unique values in each field for anything weird.
...
sort(unique(wetdat$Latin_Name)) # see all the species in the dataset
```
</details>
<br>
